...
class SakuraBubble(QtWidgets.QWidget):
    ...
    def load_icon(self):
        ...
        other_extensions = ["jpeg", "jpg", "ico"]
        
        base_filenames = ["Sakura", "icon", "app_icon"] # Common names
        
        # Check in current directory and an 'assets' subdirectory
        script_dir = os.path.dirname(os.path.abspath(__file__))
        possible_dirs = [script_dir, os.path.join(script_dir, "assets")]
    ...
    def setup_context_menu(self):
        ...
        settings_action = self.context_menu.addAction("Settings")
    ...
    def paintEvent(self, event):
        ...
        bubble_radius = min(rect.width(), rect.height()) / 2 * 0.85 

        # --- 1. Draw Shadow ---
        painter.save()
        shadow_offset_y = bubble_radius * 0.1
        ...
        current_glow_radius = bubble_radius + 5 # Glow slightly larger than bubble
        ...
        pen = QtGui.QPen(current_border_color, 2) # Border thickness
        painter.setPen(pen)
    ...
    def open_chat_window(self):
        if self.chat_window is None or not self.chat_window.isVisible():
            ...
            self.chat_window.activateWindow() # Bring to front
        else:
            self.chat_window.activateWindow()
            self.chat_window.show() # Ensure it's visible if minimized/hidden

    def on_wake_word(self): # Assuming this is called externally
        self.update_visual_state(is_listening=True)
        self.open_chat_window()
        if self.chat_window:
            self.chat_window.start_listening()
        # Glow will reset based on chat_window.listening state or timeout
        QtCore.QTimer.singleShot(4000, self.check_listening_status_after_wake)
    ...
    def close_application(self):
        ...
        # if hasattr(self, 'wake_listener') and self.wake_listener: # Original
