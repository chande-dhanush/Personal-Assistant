...
BUFFER_SIZE = int(SAMPLE_RATE * BUFFER_DURATION)  # 8000 samples
DTW_THRESHOLD = 15.0         # V4.2: Normalized avg cost threshold

# V4.3: Adaptive Noise Floor
NOISE_FLOOR_ALPHA = 0.02     # Slow EMA update rate
SPEECH_GATE_FACTOR = 2.5     # Speech must be 2.5x noise floor
INITIAL_NOISE_FLOOR = 0.01   # Starting estimate

# V4.3: Sustained Speech Gate
MIN_SPEECH_DURATION = 0.15   # 150ms minimum sustained speech
SPEECH_SAMPLES_REQUIRED = int(SAMPLE_RATE * MIN_SPEECH_DURATION / (BUFFER_SIZE / 2))  # Frames needed

# V4.3: Template Voting
MIN_TEMPLATE_MATCHES = 2     # Require ≥2 template matches for consensus

# V4.3: Timing
COOLDOWN_SECONDS = 5         # Time before next detection
REFRACTORY_SECONDS = 2.5     # Hard DTW disable after trigger
LOW_POWER_IDLE_SECONDS = 10  # Enter LOW_POWER after N seconds silence
...
class WakeWordDetector:
    ...
    def _on_mic_frame(self, samples: np.ndarray, raw_bytes: bytes):
        ...
        print(f"[WAKE] rms={rms:.3f} nf={self._noise_floor:.3f} ratio={ratio:.1f} matches={matches}/{total} best={best_distance:.1f}")
    ...
    def _dtw_distance(self, seq1: np.ndarray, seq2: np.ndarray) -> float:
        """
        Calculate NORMALIZED DTW distance between two MFCC sequences.
        
        V4.2: Returns average cost per alignment step (not raw total).
        This makes threshold values meaningful across different lengths.
        """
        ...
        # V4.2: Normalize by path length (approximate as n + m)
        raw_cost = dtw[n, m]
        path_length = n + m
        avg_cost = raw_cost / path_length
