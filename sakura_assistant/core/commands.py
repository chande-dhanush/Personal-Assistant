...
def handle_message_command(user_input, assistant_ref):
    ...
    if not message:
        if assistant_ref:
            assistant_ref.pending_action = 'whatsapp_message'
            assistant_ref.pending_whatsapp_contact = contact_name
...
def watch_anime(user_input):
    ...
    if 'open the anime' in user_input:
        user_input = user_input.replace('open anime', '').strip()
    if not user_input:
        return "Please specify an anime to watch."
    anime_name = user_input.strip()
    if not anime_name:
        return "Please provide the name of the anime you want to watch."
    # Construct the search URL for hianime.sx
    url = f"https://hianime.sx/search?keyword={anime_name}"
    
    try:
        # Open the URL in the default web browser
        webbrowser.open(url)
        return f"Searching for **{anime_name}** online."
    except Exception as e:
