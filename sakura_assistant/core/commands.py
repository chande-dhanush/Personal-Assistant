...
import threading, time, os, spotipy
from spotipy.oauth2 import SpotifyOAuth

# ---- CONFIG -------------------------------------------------
POLL_INTERVAL   = 15            # seconds between device scans
DEFAULT_DEVICE  = "LEVOS"       # rename to your favourite box
_current_device_id = None       # cache for the active device
spotify_client      = None      # will be set after auth
# -------------------------------------------------------------
...
def _device_poller():
    ...
    while True:
        if spotify_client is None:
            ...
            continue
        try:
            devs = spotify_client.devices()['devices']
            if devs:
                target = next((d for d in devs if d['name'] == DEFAULT_DEVICE), devs[0])
                _current_device_id = target['id']
                
                # 👉 Only log if it's a new device
                if last_seen_device != target['name']:
                    print(f"🎧  Using {target['name']}")
                    last_seen_device = target['name']

                if not target['is_active']:
                    spotify_client.transfer_playback(target['id'], force_play=False)
            else:
                print("🙈  No devices yet… still hunting")
        except Exception as oops:
            print("💥  Spotify poll fail:", oops)
        time.sleep(POLL_INTERVAL)

# ---- SPOTIFY AUTH & START POLLER ---------------------------
if SPOTIFY_CLIENT_ID and SPOTIFY_CLIENT_SECRET:
    try:
        ...
        try:
            ...
            if devices["devices"]:
                ...
                print("No devices yet—I'll keep stalking and grab one when it shows up.")
        except Exception as e:
            print(f"Error testing Spotify connection: {e}")

        # **Start the poller only after the client exists**
    ...
    print("⚠️  Spotify creds missing—set SPOTIFY_CLIENT_ID / SPOTIFY_CLIENT_SECRET!")
...
def handle_spotify_play_command(user_input):
    ...
    try:
        ...
        if not device_id:
            return "No Spotify devices found. Open Spotify someplace and I’ll latch on."

        # kick off playback
        spotify_client.start_playback(device_id=device_id, uris=[track['uri']])
...
def handle_command(user_input, chat_callback, assistant_ref=None):
    ...
    commands = {
        ...
        'system status': lambda: get_system_info(),
        'the time': lambda: f"The current time is {get_current_time()}",
        'the date': lambda: f"Today is {get_current_date()}",
        ...
        'i am bored': lambda: handle_fun_apis(user_input_lower),
        'search for': lambda: handle_fun_apis(user_input_lower),
        'search up': lambda: handle_fun_apis(user_input_lower),
        'the weather': lambda: handle_fun_apis(user_input_lower),
        'open mail': lambda: open_mail(),
    }
